#Taken from https://github.com/nkrakauer/Property-Predictions/blob/master/chem_scripts/skunkImage.py on April 29, 2019
# Modified to not use partial charge
...
def cs_compute_features(mol):
    ...
    # Atomic properties
    df_atom = pd.DataFrame(columns=['idx', 'amu', 'x', 'y', 'z', 'fc', 'hyb', 'val_ex', 'val_im'])
    for atom in mol.GetAtoms():
        ...
        df_atom.loc[len(df_atom)] = [float(idx), float(amu), float(x), float(y), float(z), 
                                     float(fc), float(hyb), float(val_ex), float(val_im)]
...
def cs_map_atom_to_grid(myarray, channel, df_atom, projection="2D", representation="engA"):
    ...
    for i in range(0,numatom):
        ...
        if representation == "engA": # amu+bond, bond_order, formal charge, val_ex
            eng_rep = channel[amu_grid]
            eng_rep[2] = float(df_atom['fc'][i])
            eng_rep[3] = float(df_atom['val_ex'][i])
            #print(eng_rep)
            myarray[x_grid, y_grid] = eng_rep
        elif representation == "engB": # amu, bond_order, formal charge, val_im
            eng_rep = channel[amu_grid]
            eng_rep[2] = float(df_atom['fc'][i])
            eng_rep[3] = float(df_atom['val_im'][i])
            #print(eng_rep)
            myarray[x_grid, y_grid] = eng_rep
        elif representation == "eng": # amu, bond_order, val_ex, val_im
            eng_rep = channel[amu_grid]
            eng_rep[2] = float(df_atom['val_ex'][i])
            eng_rep[3] = float(df_atom['val_im'][i])
            #print(eng_rep)
            myarray[x_grid, y_grid] = eng_rep
        elif representation == "engD": # amu+bond, formal charge, val_ex, val_im
            eng_rep = channel[amu_grid]
            eng_rep[1] = float(df_atom['fc'][i])
            eng_rep[2] = float(df_atom['val_ex'][i])
            eng_rep[3] = float(df_atom['val_im'][i])
            #print(eng_rep)
            myarray[x_grid, y_grid] = eng_rep
